...
function printTestResults(testResult) {
  ...
  if (Object.keys(testResultsGrouped).length === 0) {
    ...
    Object.keys(testResultsGrouped).map(group => {
      ...
      console.log(
        `  ${testGroupName}`,
        info(`- ${percentageTestPassed}% (${testResultsGrouped[group].passed.length } passed, ${numberOfTestsInGroup} total)`)
      )
...
function printListSchemas() {
  ...
  for (const schemaName in schemas) {
    ...
    const prevSchemaName = (schemas[Object.keys(schemas)[i - 1]]) ? schemas[Object.keys(schemas)[i - 1]].name : null
