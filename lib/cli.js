...
async function printTestResults(testResult, options) {
      ...
      if (showInfo || (
          ...
          if (Array.isArray(test.value) && test.value.length > 1) {
            ...
            if (test.error && test.error.type) {
              console.log(chalk.grey(`          └─ `),  test.autoDetected 
                ...
                : test.warning ? warn(test.error.type) : test.optional ? meta(test.error.type) : error(test.error.type)
              )
            }
        ...
        testResultsGrouped[group].optional.map(test => {
          ...
          if ((test.passed && !test.autoDetected) || (showInfo && test.value !== null)) {
            console.log(
              meta(`    • `),
              info(test.description ? test.description : test.test.replace(/"/g, '')),
            )
            if (showInfo) showTestInfo(test)
          }
        })
        ...
        testResultsGrouped[group].warnings.map(test => {
          console.log(
            warn(`    ▲ `),
            info(test.description ? test.description : test.test.replace(/"/g, '')),
            test.pending ?  chalk.grey(`\n       ⓘ  https://pending.schema.org`) : '',
            test.description ? chalk.grey(`\n       └─ ${test.test.replace(/"/g, '')}`) : '',
          )
          if (showInfo) showTestInfo(test)
        })
  ...
  console.log(info(`  Schema.org schemas:`), `${schemaOrgSchemas.join(', ') || chalk.grey('None')}`)
  console.log(info(`       Other schemas:`), `${otherSchemas.join(', ') || chalk.grey('None')}`)
  console.log(info(`     Test groups run:`), `${Object.keys(testResultsGrouped).length || 0}`)
  console.log(info(`  Properties checked:`), `${totalTests + testResult.optional.length}`)
  console.log(info(`           Tests run:`), `${totalTests}`)
  console.log('')
